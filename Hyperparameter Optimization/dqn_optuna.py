...
class DQN():
    def __init__(self, lr, eps_decay, batch_size, replace, hl1_dim, hl2_dim): # maybe replace
        ...
        self.Q = Network(self.lr, self.input_dim, self.hl1_dim, self.hl2_dim, self.output_dim)
        self.Q_next = Network(self.lr, self.input_dim, self.hl1_dim, self.hl2_dim, self.output_dim)
    ...
    def learn(self):
        ...
        q_val = torch.squeeze(self.Q.predict(states))[indices, actions] # should give action values for batch of states
        #q_val = torch.reshape(q_val, (1, self.batch_size))
        q_next = torch.squeeze(self.Q_next.predict(states_)).max(dim=1)[0]
        q_next[dones] = 0.0
...
def train_dqn_optuna(config, checkpoint_dir=None):
    ...
    agent  = DQN(lr=config["lr"], hl1_dim=config["hl1_dim"], hl2_dim=config["hl2_dim"], \
                 eps_decay=config["eps_decay"], batch_size=config["batch_size"], replace=config["replace"])

    fname = 'DQN_'+ str(config["lr"]) + '_lr_' + str(config["hl1_dim"]) + '_hl1_dim_' + \
...
if __name__ == '__main__':
    ...
    results = tune.run(
        train_dqn_optuna, # Specify the algorithm to train
        config=config,
    ...
    df.to_csv("dqn_optunasearch_64_v1.csv")
