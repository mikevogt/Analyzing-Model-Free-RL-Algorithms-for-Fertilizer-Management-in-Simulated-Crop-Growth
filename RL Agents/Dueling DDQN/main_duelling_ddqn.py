...
from duelling_architectures import Duelling_DDQN
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = 'Dueling DDQN Agent')
    parser.add_argument('-lr', type=float, default=0.00097369, help='learning rate for agent')
    parser.add_argument('-hl1_dim', type=int, default=752, help='hidden layer 1 dimension')
    ...
    parser.add_argument('-batch_size', type=int, default=384, help='size of mini-batch to sample from replay memory')
    parser.add_argument('-replace', type=int, default=50, help='replace values of target network with values from behaviour network')
    ...
    fname = 'Duelling_DDQN_' + str(args.lr) + '_lr_' + str(args.hl1_dim) + '_hl1_' + str(args.hl2_dim) + '_hl2_' + str(args.num_episodes) + '_episodes_'\
    ...
    agent = Duelling_DDQN(lr=args.lr, eps_decay=args.eps_decay, eps_min=args.eps_min, gamma=args.gamma, capacity=100000, batch_size=args.batch_size, replace=args.replace,\
